...
def preprocess_text(text):
    ...
    text = re.sub(r'[^\x00-\x7f]', r'', text)   
            # Pembersihan Karakter (Hapus angka dan non-huruf, hanya menyisakan huruf (a-z) dan spasi)
    text = re.sub(r'[^a-z\s]', ' ', text)    
            # Hapus spasi ganda dan spasi di awal/akhir
    ...
    # 6. Koreksi Over-Stemming (Contoh: 'cuku' -> 'cukup')
    final_tokens = []
...
def process_and_save_dataset(file_name, text_column_name='konten', title_column_name='judul', max_rows=None, enable_demo=True):
    ...
    # Menambahkan Kolom Baru untuk Teks yang Sudah Bersih
    df['konten_bersih'] = ''
    ...
    for index, row in df.iterrows():
        ...
        # LOGIKA PROGRESS BAR DENGAN MENCETAK KELIPATAN 5%
